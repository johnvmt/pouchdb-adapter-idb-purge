...
function compactRevs(revs, docId, txn) {
  ...
  revs.forEach(function (rev) {
    ...
    var getKEY = index.getKey(key)

    getKEY.onerror = function(e) {
      console.log("ERR",e);
    }


    getKEY.onsuccess = function (e) {
      var seq = e.target.result;
      if (typeof seq !== 'number') {
        return checkDone();
      }

      console.log("DEL-SEQ",seq);
      seqStore.delete(seq);
...
function init(api, opts, callback) {
  ...
  api.purge = pouchdbUtils.toPromise(
    function (docOrId, callback) {
      let docId = (typeof docOrId == 'object' && typeof docOrId._id != 'undefined') ? docOrId._id : docOrId;
